...
describe("updateDocumentMetadataById module", () => {
  let sandbox;
  let updateDocumentMetadataById;
  let DocumentMetadataStub, ElasticIndexerStub;

  const fakeId = "uuid-123";
  const fakeUpdatedData = { id: fakeId, name: "Updated DocumentMetadata" };
  ...
  beforeEach(() => {
    ...
    updateDocumentMetadataById = proxyquire(
      "../../../../../src/db-layer/main/DocumentMetadata/utils/updateDocumentMetadataById",
      {
  ...
  describe("updateDocumentMetadataById", () => {
    it("should update record by direct ID", async () => {
      const result = await updateDocumentMetadataById(fakeId, {
        name: "Updated",
      });
    ...
    it("should extract ID from dataClause if not explicitly provided", async () => {
      const result = await updateDocumentMetadataById(undefined, {
        id: fakeId,
        name: "Updated",
      });
      expect(result).to.deep.equal(fakeUpdatedData);
    ...
    it("should support object-as-ID input", async () => {
      const result = await updateDocumentMetadataById(
        { id: fakeId },
        { name: "Updated" },
      );
      expect(result).to.deep.equal(fakeUpdatedData);
    ...
    it("should throw BadRequestError if no ID is provided", async () => {
      try {
        await updateDocumentMetadataById(undefined, {});
    ...
    it("should throw NotFoundError if no existing record is found", async () => {
      ...
      try {
        await updateDocumentMetadataById(fakeId, { name: "Updated" });
        throw new Error("Expected error");
      } catch (err) {
        expect(err.name).to.equal("HttpServerError");
    ...
    it("should throw HttpServerError if findOne fails", async () => {
      DocumentMetadataStub.findOne.rejects(new Error("DB fail"));
      try {
        await updateDocumentMetadataById(fakeId, { name: "Updated" });
        throw new Error("Expected error");
      } catch (err) {
