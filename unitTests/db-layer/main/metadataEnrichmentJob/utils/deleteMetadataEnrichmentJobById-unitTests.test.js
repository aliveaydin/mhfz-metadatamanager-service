...
describe("deleteMetadataEnrichmentJobById module", () => {
  let sandbox;
  let deleteMetadataEnrichmentJobById;
  ...
  beforeEach(() => {
    ...
    mockDocInstance = {
      ...
      getData: () => ({ id: fakeId, name: "Deleted MetadataEnrichmentJob" }),
    };
    ...
    deleteMetadataEnrichmentJobById = proxyquire(
      "../../../../../src/db-layer/main/MetadataEnrichmentJob/utils/deleteMetadataEnrichmentJobById",
      {
  ...
  describe("deleteMetadataEnrichmentJobById", () => {
    it("should delete MetadataEnrichmentJob and index removal in Elastic", async () => {
      const result = await deleteMetadataEnrichmentJobById(fakeId);

      expect(result).to.deep.equal({
        id: fakeId,
        name: "Deleted MetadataEnrichmentJob",
      });
    ...
    it("should support object as ID input", async () => {
      ...
      MetadataEnrichmentJobStub.findOne.resolves(localMock);

      const result = await deleteMetadataEnrichmentJobById({ id: fakeId });
    ...
    it("should throw BadRequestError if no id is provided", async () => {
      try {
        await deleteMetadataEnrichmentJobById(undefined);
    ...
    it("should throw NotFoundError if record is not found", async () => {
      ...
      try {
        await deleteMetadataEnrichmentJobById(fakeId);
    ...
    it("should wrap internal errors with HttpServerError", async () => {
      MetadataEnrichmentJobStub.findOne.rejects(new Error("unexpected error"));
    ...
    it("should wrap ElasticIndexer.deleteData() error with HttpServerError", async () => {
      ...
      const result = await deleteMetadataEnrichmentJobById(fakeId).catch(
        (err) => err,
      );
    ...
    it("should accept numeric ID as valid input", async () => {
      ...
      const result = await deleteMetadataEnrichmentJobById(numericId);
      expect(result).to.deep.equal({
        id: numericId,
        name: "Deleted MetadataEnrichmentJob",
      });
