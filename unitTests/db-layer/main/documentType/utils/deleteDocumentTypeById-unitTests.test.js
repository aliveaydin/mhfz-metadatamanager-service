...
describe("deleteDocumentTypeById module", () => {
  let sandbox;
  let deleteDocumentTypeById;
  let DocumentTypeStub, ElasticIndexerStub;
  let mockDocInstance;
  ...
  beforeEach(() => {
    ...
    mockDocInstance = {
      ...
      getData: () => ({ id: fakeId, name: "Deleted DocumentType" }),
    };
    ...
    deleteDocumentTypeById = proxyquire(
      "../../../../../src/db-layer/main/DocumentType/utils/deleteDocumentTypeById",
      {
  ...
  describe("deleteDocumentTypeById", () => {
    it("should delete DocumentType and index removal in Elastic", async () => {
      const result = await deleteDocumentTypeById(fakeId);

      expect(result).to.deep.equal({
        id: fakeId,
        name: "Deleted DocumentType",
      });
    ...
    it("should support object as ID input", async () => {
      ...
      DocumentTypeStub.findOne.resolves(localMock);

      const result = await deleteDocumentTypeById({ id: fakeId });
    ...
    it("should throw BadRequestError if no id is provided", async () => {
      try {
        await deleteDocumentTypeById(undefined);
    ...
    it("should throw NotFoundError if record is not found", async () => {
      ...
      try {
        await deleteDocumentTypeById(fakeId);
    ...
    it("should wrap internal errors with HttpServerError", async () => {
      DocumentTypeStub.findOne.rejects(new Error("unexpected error"));
    ...
    it("should wrap ElasticIndexer.deleteData() error with HttpServerError", async () => {
      ...
      const result = await deleteDocumentTypeById(fakeId).catch((err) => err);
    ...
    it("should accept numeric ID as valid input", async () => {
      ...
      const localMock = {
        id: numericId,
        update: sandbox.stub().resolves(),
        getData: () => ({ id: numericId, name: "Deleted DocumentType" }),
      };
      DocumentTypeStub.findOne.resolves(localMock);

      const result = await deleteDocumentTypeById(numericId);
      expect(result).to.deep.equal({
        id: numericId,
        name: "Deleted DocumentType",
      });
