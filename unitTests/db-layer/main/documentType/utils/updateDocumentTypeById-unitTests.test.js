...
describe("updateDocumentTypeById module", () => {
  let sandbox;
  let updateDocumentTypeById;
  let DocumentTypeStub, ElasticIndexerStub;

  const fakeId = "uuid-123";
  const fakeUpdatedData = { id: fakeId, name: "Updated DocumentType" };
  ...
  beforeEach(() => {
    ...
    updateDocumentTypeById = proxyquire(
      "../../../../../src/db-layer/main/DocumentType/utils/updateDocumentTypeById",
      {
  ...
  describe("updateDocumentTypeById", () => {
    it("should update record by direct ID", async () => {
      const result = await updateDocumentTypeById(fakeId, { name: "Updated" });
    ...
    it("should extract ID from dataClause if not explicitly provided", async () => {
      const result = await updateDocumentTypeById(undefined, {
        id: fakeId,
        name: "Updated",
      });
      expect(result).to.deep.equal(fakeUpdatedData);
    ...
    it("should support object-as-ID input", async () => {
      const result = await updateDocumentTypeById(
        { id: fakeId },
        { name: "Updated" },
      );
      expect(result).to.deep.equal(fakeUpdatedData);
    ...
    it("should throw BadRequestError if no ID is provided", async () => {
      try {
        await updateDocumentTypeById(undefined, {});
    ...
    it("should throw NotFoundError if no existing record is found", async () => {
      ...
      try {
        await updateDocumentTypeById(fakeId, { name: "Updated" });
        throw new Error("Expected error");
      } catch (err) {
        expect(err.name).to.equal("HttpServerError");
    ...
    it("should throw HttpServerError if findOne fails", async () => {
      DocumentTypeStub.findOne.rejects(new Error("DB fail"));
      try {
        await updateDocumentTypeById(fakeId, { name: "Updated" });
        throw new Error("Expected error");
      } catch (err) {
