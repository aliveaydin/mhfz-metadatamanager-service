...
describe("mhfz-session", () => {
  beforeEach(() => {
    InstanceSession = proxyquire("../../src/project-session/mhfz-session", {
      common: {
  ...
  describe("checkTokenLocations", () => {
    ...
    it("should return token from static header", async () => {
      const req = { query: {}, headers: { "mhfz-access-token": "tok123" } };
      const result = await instance.checkTokenLocations(req);
      expect(result).to.deep.equal([
        "tok123",
        false,
    ...
    it("should return token from static cookie", async () => {
      ...
      instance.getCookieToken = sinon
        .stub()
        .withArgs("mhfz-access-token", req)
        .returns("cookie123");
      const result = await instance.checkTokenLocations(req);
      expect(result).to.deep.equal([
        "cookie123",
