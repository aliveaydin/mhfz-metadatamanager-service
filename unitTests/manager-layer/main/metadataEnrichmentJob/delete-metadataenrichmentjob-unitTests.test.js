...
describe("DeleteMetadataenrichmentjob", () => {
  let ControllerClass;
  let req;
  ...
  beforeEach(() => {
    ...
    ControllerClass = proxyquire(
      "../../../../src/manager-layer/main/MetadataEnrichmentJob/delete-metadataenrichmentjob",
      {
  ...
  it("should initialize controller with correct base properties", () => {
    ...
    expect(instance.options.name).to.equal("deleteMetadataenrichmentjob");
    expect(instance.options.controllerType).to.equal("rest");
    expect(instance.options.crudType).to.equal("delete");
    expect(instance.dataName).to.equal("metadataEnrichmentJob");
  });
});
