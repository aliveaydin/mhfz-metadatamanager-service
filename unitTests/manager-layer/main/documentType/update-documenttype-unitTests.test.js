...
describe("UpdateDocumenttype", () => {
  ...
  beforeEach(() => {
    ...
    ControllerClass = proxyquire(
      "../../../../src/manager-layer/main/DocumentType/update-documenttype",
      {
  ...
  it("should initialize controller with correct base properties", () => {
    ...
    expect(instance.options.name).to.equal("updateDocumenttype");
    expect(instance.options.controllerType).to.equal("rest");
    expect(instance.options.crudType).to.equal("update");
    expect(instance.dataName).to.equal("documentType");
  });
});
