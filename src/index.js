...
console.log("Starting mhfz-metadatamanager-service service...");
...
const start = async () => {
  process.title = "node-mhfz-metadatamanager-service";
  ...
  try {
    ...
    await setRedisData(
      "mhfz-metadatamanager-service:start",
      today.toUTCString(),
    );
    console.log(
      "Service start datetime is written to Redis key mhfz-metadatamanager-service:start",
      today.toUTCString(),
    );
  } catch (err) {
  ...
  console.log(
    "mhfz-metadatamanager-service is listening HTTP/REST port " +
...
/* starting the server mhfz-metadatamanager-service .... */
start();
