...
const indexDataToElastic = async (data) => {
  const elasticIndexer = new ElasticIndexer("documentMetadata");
  await elasticIndexer.indexData(data);
};

const updateDocumentMetadataById = async (id, dataClause) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const [rowsCount, [dbDoc]] = await DocumentMetadata.update(
      dataClause,
      options,
    );
    if (!dbDoc) {
...
module.exports = updateDocumentMetadataById;
