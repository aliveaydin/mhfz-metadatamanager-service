...
const indexDataToElastic = async (data) => {
  const elasticIndexer = new ElasticIndexer("metadataEnrichmentJob");
  await elasticIndexer.indexData(data);
};

const updateMetadataEnrichmentJobById = async (id, dataClause) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const existingDoc = await MetadataEnrichmentJob.findOne({
    ...
    const [rowsCount, [dbDoc]] = await MetadataEnrichmentJob.update(
      dataClause,
      options,
    );
    if (!dbDoc) {
...
module.exports = updateMetadataEnrichmentJobById;
