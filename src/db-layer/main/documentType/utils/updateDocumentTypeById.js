...
const updateDocumentTypeById = async (id, dataClause) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const [rowsCount, [dbDoc]] = await DocumentType.update(dataClause, options);
    if (!dbDoc) {
...
module.exports = updateDocumentTypeById;
